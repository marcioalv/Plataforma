...
// Descrição: Formulário com o cadastro de ações com registros.
//
...
const
  ...
  ERRO_MENSAGEM    : string = 'Impossível gravar dados da ação em registro!';
var
  locADOConnection       : TADOConnection;
  locADOQuery            : TADOQuery;
  locLogMensagem         : string;
...
begin
  ...
  if locCodigo = '' then
  begin
    VCLConsistenciaExibir('O código da ação em registro deve ser informado!');
    VCLPageControlFocar(pagFormulario, TAB_CADASTRO, edtCodigo);
    Exit;
  end;
  ...
  begin
    VCLConsistenciaExibir('A descrição da ação em registro deve ser informada!');
  ...
  begin
    VCLConsistenciaExibir('Um tipo de ação deve ser selecionado!');
  ...
  //
  // Determina se o código da ação em registro já existe no banco de dados para um outro registro.
  //
  locADOQuery.Close;
  locADOQuery.SQL.Clear;
  ...
  try
    ...
    begin
      ...
      locADOConnection.Close;
      FreeAndNil(locADOConnection);
      locLogMensagem := 'Ocorreu algum problema ao executar query para selecionar o mesmo código da ação em registro em um outro registro!';
      Plataforma_ERP_Logar(True, ERRO_MENSAGEM, locLogMensagem, locExcecao.Message, FONTE_NOME, PROCEDIMENTO_NOME);
      VCLErroExibir(ERRO_MENSAGEM, locLogMensagem, locExcecao.Message);
      Exit;
    end;
  ...
  begin
    ...
    locADOConnection.Close;
    FreeAndNil(locADOConnection);
    locLogMensagem := 'O código "' + locCodigo + '" informado para a ação em registro já existe em algum outro cadastro!';
  ...
  begin
    ...
    try
      ...
      begin
        ...
        locADOConnection.Close;
        FreeAndNil(locADOConnection);
        locLogMensagem := 'Ocorreu algum erro ao executar o comando SQL para consultar se o contador de atualizações confere na tabela [registro_acao]!';
        Plataforma_ERP_Logar(True, ERRO_MENSAGEM, locLogMensagem, locExcecao.Message, FONTE_NOME, PROCEDIMENTO_NOME);
        VCLErroExibir(ERRO_MENSAGEM, locLogMensagem, locExcecao.Message);
        Exit
      end;
  ...
  begin
    try
      ...
      begin
        ...
        locADOConnection.Close;
        FreeAndNil(locADOConnection);
        locLogMensagem := 'Impossível determinar o próximo numerador para a ação em registro!';
        Plataforma_ERP_Logar(True, ERRO_MENSAGEM, locLogMensagem, locExcecao.Message, FONTE_NOME, PROCEDIMENTO_NOME);
        VCLErroExibir(ERRO_MENSAGEM, locLogMensagem, locExcecao.Message);
        Exit
      end;
  ...
  //
  // Ação em registro gravada!
  //
  VCLInformacaoExibir('Ação em registro gravada com sucesso!');
end;
...
const
  ...
  ERRO_MENSAGEM    : string = 'Impossível excluir dados da ação em registro!';
var
  locADOConnection       : TADOConnection;
  locADOQuery            : TADOQuery;
  locLogMensagem         : string;
...
begin
  ...
  //
  try
    Plataforma_ERP_ADO_LogOcorrenciaInserir(REGISTRO_ACAO_EXCLUSAO, locRegistroAcaoID, edtCodigo.Text, 'registro_acao', 'Registro excluído com sucesso!', locRegistroAcaoLogDados);
  except
  end;
  VCLInformacaoExibir('Ação em registro excluída com sucesso!');
end;
...
begin
  ...
  try
    ...
    begin
      ...
      locADOConnection.Close;
      FreeAndNil(locADOConnection);
      locLogMensagem := 'Ocorreu algum erro ao executar o comando SQL para consultar último cadastro na tabela [registro_acao]!';
      Plataforma_ERP_Logar(True, ERRO_MENSAGEM, locLogMensagem, locExcecao.Message, FONTE_NOME, PROCEDIMENTO_NOME);
      VCLErroExibir(ERRO_MENSAGEM, locLogMensagem, locExcecao.Message);
      Exit;
    end;
