...
// Descrição: Formulário com o cadastro de numeradores por base.
//
...
const
  ...
  ERRO_MENSAGEM    : string = 'Impossível gravar dados do numerador por base!';
var
  locADOConnection        : TADOConnection;
  locADOQuery             : TADOQuery;
  locLogMensagem          : string;
...
begin
  ...
  if locCodigo = '' then
  begin
    VCLConsistenciaExibir('O código do numerador por base deve ser informado!');
    VCLPageControlFocar(pagFormulario, TAB_CADASTRO, edtCodigo);
    Exit;
  end;
  ...
  begin
    VCLConsistenciaExibir('O ID atual do numerador por base não deve ser um número negativo!');
  ...
  //
  // Determina se o código do numerador por base já existe no banco de dados para um outro registro.
  //
  if locInsert then
  begin
    locADOQuery.Close;
    ...
    try
      ...
      begin
        ...
        locADOConnection.Close;
        FreeAndNil(locADOConnection);
        locLogMensagem := 'Ocorreu algum problema ao executar query para selecionar o mesmo código de numerador por base em um outro registro!';
        Plataforma_ERP_Logar(True, ERRO_MENSAGEM, locLogMensagem, locExcecao.Message, FONTE_NOME, PROCEDIMENTO_NOME);
        VCLErroExibir(ERRO_MENSAGEM, locLogMensagem, locExcecao.Message);
        Exit;
      end;
    ...
    begin
      ...
      locADOConnection.Close;
      FreeAndNil(locADOConnection);
      locLogMensagem := 'O código "' + locCodigo + '" informado para o numerador por base já existe em algum outro cadastro!';
  ...
  begin
    ...
    try
      ...
      begin
        ...
        locADOConnection.Close;
        FreeAndNil(locADOConnection);
        locLogMensagem := 'Ocorreu algum erro ao executar o comando SQL para consultar se o contador de atualizações confere na tabela [numerador_base]!';
        Plataforma_ERP_Logar(True, ERRO_MENSAGEM, locLogMensagem, locExcecao.Message, FONTE_NOME, PROCEDIMENTO_NOME);
        VCLErroExibir(ERRO_MENSAGEM, locLogMensagem, locExcecao.Message);
        Exit
      end;
...
const
  ...
  ERRO_MENSAGEM    : string = 'Impossível excluir dados do numerador por base!';
var
  locADOConnection        : TADOConnection;
  locADOQuery             : TADOQuery;
  locLogMensagem          : string;
...
begin
  ...
  //
  try
    Plataforma_ERP_ADO_LogOcorrenciaInserir(REGISTRO_ACAO_EXCLUSAO, 0, locCodigo, 'numerador_base', 'Registro excluído com sucesso!', locNumeradorBaseLogDados);
  except
  end;
  VCLInformacaoExibir('Numerador por base excluído com sucesso!');
end;
